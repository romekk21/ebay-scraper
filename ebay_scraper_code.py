...
def parse_ebay():
    ...
    # Used in order to get rid of emojis in title
    non_bmp_map = dict.fromkeys(range(0x10000, sys.maxunicode + 1), 0xfffd)

    # Used in order to get past page 5 - Otherwise eBay flags for automation
...
def data_cleanup(titles):

    title_data = dict()

    # Words we don't care about
    stopwords = ["my", "is", "no", "i","ebay","shop","women","for", "/", "to", "~","a", "men’s", "men's", "&", "and","tshirt","mens", "men", "tee", "vintage", "vtg", "on", "of", "the", "t-shirt", "shirt", "shirt,", "sz", "size", "single", "-", "all", "in", "t"]

    # For each FULL title of a listing
    for title in titles:

        # Split into a list of the words that make up the title
        words_in_title = title.split()
        
        # For each word in the title of one listing 
